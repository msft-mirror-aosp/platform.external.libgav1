...
LIBGAV1_ALWAYS_INLINE void DoHorizontalPass2xH(
  ...
  if (filter_index == 4) {  // 4 tap.
    dsp::SetupTaps<4>(&v_horizontal_filter, v_tap);
    ...
    dsp::SetupTaps<2>(&v_horizontal_filter, v_tap);
...
void Convolve2D_AVX2(const void* const reference,
  ...
  if (width > 8) {
    ...
    // Use 128 bit code.
    if (vertical_taps == 8) {
      dsp::SetupTaps<8, /*is_2d_vertical=*/true>(&v_filter, taps);
      if (width == 2) {
      ...
      dsp::SetupTaps<6, /*is_2d_vertical=*/true>(&v_filter, taps);
      if (width == 2) {
      ...
      dsp::SetupTaps<4, /*is_2d_vertical=*/true>(&v_filter, taps);
      if (width == 2) {
      ...
      dsp::SetupTaps<2, /*is_2d_vertical=*/true>(&v_filter, taps);
      if (width == 2) {
...
void FilterVertical8xH(const uint8_t* src, const ptrdiff_t src_stride,
  ...
  do {
    ...
    if (is_compound) {
      const __m128i results = dsp::Compound1DShift(sums);
